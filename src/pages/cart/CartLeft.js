...
const CartLeft = () => {
  ...
  return (
    <div className={cart.length > 0 ? "cart_left" : "cart_left not"}>
      {/* mobile-checkout */}
      <div
        className="cart_right_three_bottom_mobile"
      ...
      {/* title */}
      <div className="cart_left_title">My Cart</div>

      {/* content */}
      {cart && cart.length > 0 ? (
        <>
          {/* two */}
          <div className="cart_left_two">
            {/* content */}
            {cart &&
              cart.length > 0 &&
              cart.map((item, i) => (
                <div className="cart_left_two_content_div" key={i}>
                  <div className="cart_left_two_content">
                    <div
                      className="cart_left_two_content_one"
                      onClick={() => {
                    ...
                    >
                      ...
                      <div className="cart_left_two_content_arrow">
                        <div className="cart_left_two_content_arrow_icon">
                          <CgPushDown />
                        </div>
                      </div>
                    </div>

                    <div className="cart_left_two_content_two">
                      <p>{item?.title}</p>
                      <p className="cart_left_two_content_two_price">{`₹${numeral(
                        item?.price
                      ).format("0,0.00")}`}</p>

                      <div className="cart_left_content_two_mobile">
                        <span
                          className="cart_left_two_content_three_dec"
                        ...
                        <span
                          className="cart_left_two_content_three_inc"
                    ...
                    <div className="cart_left_two_content_three">
                      <span
                        className="cart_left_two_content_three_dec"
                        onClick={() => {
                          decItem(item);
                    ...
                    <div className="cart_left_two_content_four">{`₹${numeral(
                      item?.price * item?.count
                    ).format("0,0.00")}`}</div>
                    <div className="cart_left_two_content_five">
                      <AiOutlineClose onClick={() => handleRemove(item)} />
                    </div>

                    {/* mobile-close */}
                    <div className="cart_left_content_two_mobile_div">
                      <div className="cart_left_two_content_five_mobile">
                        <AiOutlineClose onClick={() => handleRemove(item)} />
                      </div>
                      <div className="cart_left_content_two_mobile_price">{`₹${numeral(
                        item?.price * item?.count
                      ).format("0,0.00")}`}</div>
                    </div>
                  </div>
        ...
        <div className="cart_left_no_cart">
          <p className="cart_left_no_cart_top">Cart is Empty</p>
          <Link to="/courses">
            <p className="cart_left_no_cart_bottom">Continue Shopping</p>
          </Link>
        </div>
      )}
    </div>
